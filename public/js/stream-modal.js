...
async function loadGalleryVideos() {
  try {
    ...
    container.innerHTML =
      '<div class="text-center py-3"><i class="ti ti-loader animate-spin mr-2"></i>Loading videos...</div>';
...
function displayFilteredVideos(videos) {
  ...
  if (videos && videos.length > 0) {
    videos.forEach(video => {
      ...
      button.type = "button";
      button.className =
        "w-full flex items-center space-x-3 p-2 rounded hover:bg-dark-600 transition-colors";
...
function initModal() {
  ...
  if (videoSelectorDropdown) {
    document.addEventListener("click", e => {
      ...
      const isClickOnButton = e.target.closest(
        '[onclick="toggleVideoSelector()"]'
      );
  ...
  fetch(`/api/streams/check-key?key=${encodeURIComponent(streamKey)}`)
    ...
    .then(data => {
      ...
      if (data.isInUse) {
        ...
        streamKeyInput.classList.remove(
          "border-gray-600",
