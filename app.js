...
const videoUpload = multer({
  ...
  fileFilter: function (req, file, cb) {
    if (!file.mimetype.match(/^video\/(mp4|avi|quicktime)$/)) {
      return cb(
        new Error("Only MP4, AVI, and MOV video files are allowed!"),
        false
      );
    }
    cb(null, true);
...
app.post(
  ...
  async (req, res) => {
    try {
      ...
      const thumbnailRelativePath = await generateThumbnail(
        videoPath,
        thumbnailName
      )
        ...
        ffmpeg.ffprobe(fullFilePath, (err, metadata) => {
          ...
          if (videoStream && videoStream.avg_frame_rate) {
            ...
            if (fpsRatio.length === 2 && parseInt(fpsRatio[1]) !== 0) {
              fps =
                Math.round(
                  (parseInt(fpsRatio[0]) / parseInt(fpsRatio[1])) * 100
                ) / 100;
            } else {
  ...
  async (req, res) => {
    try {
      ...
      if (video.user_id !== req.session.userId) {
        return res
          .status(403)
          .json({ error: "You don't have permission to rename this video" });
      }
...
app.get("/stream/:videoId", isAuthenticated, async (req, res) => {
  try {
    ...
    if (video.user_id !== req.session.userId) {
      return res
        .status(403)
        .send("You do not have permission to access this video");
    }
    const videoPath = path.join(__dirname, "public", video.filepath);
...
app.get("/api/settings/gdrive-status", isAuthenticated, async (req, res) => {
  try {
    ...
    res.json({
      ...
      message: user.gdrive_api_key
        ? "Google Drive API key is configured"
        : "No Google Drive API key found",
    });
  } catch (error) {
...
app.get(
  "/api/videos/import-status/:jobId",
  isAuthenticated,
  async (req, res) => {
    ...
    if (!importJobs[jobId]) {
      return res
        .status(404)
        .json({ success: false, error: "Import job not found" });
    }
    return res.json({
      success: true,
...
async function processGoogleDriveImport(jobId, apiKey, fileId, userId) {
  ...
  try {
    ...
    const thumbnailName =
      path.basename(result.filename, path.extname(result.filename)) + ".jpg";
    const thumbnailRelativePath = await generateThumbnail(
    ...
    const videoData = {
      title: path.basename(
        ...
        path.extname(result.originalFilename)
      ),
...
app.post(
  ...
  async (req, res) => {
    try {
      ...
      const isInUse = await Stream.isStreamKeyInUse(
        req.body.streamKey,
        req.session.userId
      );
      if (isInUse) {
        return res.status(400).json({
          success: false,
          error:
            "This stream key is already in use. Please use a different key.",
        });
      }
      let platform = "Custom";
      ...
      const streamData = {
        ...
        loop_video:
          req.body.loopVideo === "true" || req.body.loopVideo === true,
        use_advanced_settings:
          req.body.useAdvancedSettings === "true" ||
          req.body.useAdvancedSettings === true,
        user_id: req.session.userId,
      };
      if (req.body.scheduleTime) {
        ...
        console.log(
          `[CREATE STREAM] Parsed time: ${scheduleDate.toISOString()}`
        );
        console.log(
          `[CREATE STREAM] Local display: ${scheduleDate.toLocaleString("en-US", { timeZone: serverTimezone })}`
        );
      ...
      res
        .status(500)
        .json({ success: false, error: "Failed to create stream" });
    }
  }
);
...
app.put("/api/streams/:id", isAuthenticated, async (req, res) => {
  try {
    ...
    if (req.body.loopVideo !== undefined) {
      updateData.loop_video =
        req.body.loopVideo === "true" || req.body.loopVideo === true;
    }
    if (req.body.useAdvancedSettings !== undefined) {
      updateData.use_advanced_settings =
        req.body.useAdvancedSettings === "true" ||
        req.body.useAdvancedSettings === true;
    }
    if (req.body.scheduleTime) {
      ...
      console.log(
        `[UPDATE STREAM] Local display: ${scheduleDate.toLocaleString("en-US", { timeZone: serverTimezone })}`
      );
  ...
  async (req, res) => {
    try {
      ...
      if (newStatus === "live") {
        ...
        const result = await Stream.updateStatus(
          streamId,
          "offline",
          req.session.userId
        );
      ...
      res
        .status(500)
        .json({ success: false, error: "Failed to update stream status" });
    }
  }
);
...
app.get("/api/streams/:id/logs", isAuthenticated, async (req, res) => {
  try {
    ...
    res
      .status(500)
      .json({ success: false, error: "Failed to fetch stream logs" });
  }
});
...
app.listen(port, "0.0.0.0", async () => {
  ...
  try {
    ...
    if (streams && streams.length > 0) {
      console.log(
        `Resetting ${streams.length} live streams to offline state...`
      );
