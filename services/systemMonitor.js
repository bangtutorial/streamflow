...
function calculateNetworkSpeed(networkData) {
  ...
  const currentTotal = networkData
    .filter(
      iface => !iface.iface.includes("lo") && !iface.iface.includes("Loopback")
    )
    .reduce(
      (acc, iface) => ({
  ...
  const downloadBps = Math.max(
    0,
    (currentTotal.rx_bytes - previousTotal.rx_bytes) / timeDiff
  );
  const uploadBps = Math.max(
    0,
    (currentTotal.tx_bytes - previousTotal.tx_bytes) / timeDiff
  );
...
async function getDiskUsage() {
  try {
    ...
    if (platform === "win32") {
      ...
      targetDisk = fsSize.find(
        disk => disk.mount.charAt(0).toUpperCase() === currentDrive
      );

      if (!targetDisk) {
        targetDisk = fsSize.find(
          disk => disk.mount.charAt(0).toUpperCase() === "C"
        );
      }
    } else {
    ...
    const usagePercent =
      targetDisk.size > 0
        ? Math.round(
            ((targetDisk.size - targetDisk.available) / targetDisk.size) * 100
          )
        : 0;
